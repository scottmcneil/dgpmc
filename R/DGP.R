...
#' Generate multi-way grouped random data ala CGM 2011
#'
#' @param dims Integer indicating the number of group dimensions
#' @param groups Vector of dims length with integers indicating total groups for each group dimension
#' @param rho Float indicating portion of first dimension groups for which cells wil have theta observations instead of 1
...
#' @return dataframe of group-correlated random values
#' @export
cgm_multiway_DGP <- function(num_dims, groups, rho = 0, theta = 1, dim_names = rev(LETTERS)[1:num_dims+18], heterosked = FALSE){
  ...
  #Generate individual level x-values
  ind_level_x <- mapply(function(name, n) setNames(data.frame(rnorm(n)), paste0('x_', name, 'i')),
                          name = names(groups_list), MoreArgs = list(n = nrow(group_combinations)), SIMPLIFY = FALSE)

  x_ind_data <- Reduce(f = cbind, x = ind_level_x, init = group_combinations)

  #Combine x-values
  x_data <- Reduce(f = merge, x = group_level_x, init = x_ind_data)

  #Sum up x-values across rows
  raw_W <- mapply(function(name, x_data) rowSums(x_data[grepl(pattern = paste0('x_',name), names(x_data))]), name = names(groups_list),
              MoreArgs = list(x_data = x_data))

  W <- setNames(data.frame(raw_W), paste0('w_', names(groups_list)))
  ...
  #Add heteroskedasticity if TRUE
  sd <- if(heterosked) 3*abs(rowSums(W)) else 1
  ...
  #Sum up all e values and W
  Y <- rowSums(W) + rowSums(e_data[grepl(pattern = 'e_', names(e_data))])
